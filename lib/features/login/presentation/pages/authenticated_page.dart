...
class _AuthenticatedPageState extends State<AuthenticatedPage>
  ...
  final authUser = StorageService().getAuthSession;
  final bool biometricActif = StorageService().getBiometrieIsActif;
  ...
  Future<void> _getAutBiometrie() async {
    try {
      await auth.initDevice(); // Attendre que l'initialisation se termine
    } catch (e) {
      Logger().i("Erreur lors de l'initialisation : $e");
    }
  }
      ...
      child: Scaffold(
        ...
        body: BlocListener<AuthBloc, AuthState>(
          listenWhen: (previous, current) =>
              ...
              current is BiometricIsSupportedSate ||
              current is BiometricNotSupportedState ||
              current is BiometricAuthenticationState ||
              current is BiometricAuthFailed,
          listener: (context, state) {
            ...
            if (state is BiometricNotSupportedState) {
              koriScaffoldMessage(context, state.notSupported,
                  statut: Statut.ERROR);
            }
            if (state is BiometricAuthenticationState) {
              koriScaffoldMessage(context, "Authentication avec succès");
            }
          },
          child: SafeArea(
            ...
            child: Column(
              ...
              children: [
                ...
                Center(
                  child: Text(
                    'Hello  ${authUser?.profile.user?.firstname ?? ""} !',
                    style: koriTextStyle(
  ...
  Widget _buildNumberPad() {
    return GridView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        if (index == 9 && biometricActif) {
          return _buildConfirmButton(() async {
            final isLogin = await auth.authenticate();
            if (!mounted) return;
            // ignore: use_build_context_synchronously
            context.read<AuthBloc>().add(BiometrieAuthEvent(isLogin: isLogin));
          });
        }
        if (index == 9) {
          return const SizedBox.shrink();
